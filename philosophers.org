...
How would you model the forks? They don't do anything, really. They have a position and are either in use or not in use. If in use, do we record (there) with philosoper is using the fork? Just that it is in use (reserved, has a mutex on it?)
...
varaibles, structs. What do they hold?
